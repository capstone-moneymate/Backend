...
> 이 문서는 팀의 협업 규칙(커밋·브랜치·PR)을 한곳에 모은 가이드입니다.
> 프론트엔드 저장소와 동일한 규칙을 사용합니다.
>
> 현재는 **권장 규칙**이며, 자동 검사와 GitHub 보호 규칙(브랜치 보호, CI)은 추후 도입 예정입니다. 도입되면 이 문단을 갱신합니다.
> 코드 컨벤션(네이밍, 포맷터, 린트)은 기술 스택이 확정된 뒤 추가합니다.
...
1. GitHub에서 **이슈를 생성**합니다. (템플릿 선택)
   - `feat` / `fix` / `refactor`는 각각의 템플릿을 씁니다.
...
5. **팀원 승인 1명 + CI 통과** 후 **Squash and merge** 합니다.
6. 머지된 작업 브랜치는 삭제합니다.

> `main`과 `develop`에는 직접 push하지 않고, 반드시 PR을 통해 머지합니다.
...
| 타입       | 의미                      | 예시                            |
| :--------- | :------------------------ | :------------------------------ |
| `feat`     | 새로운 기능 추가          | API 구현, 도메인 로직 추가      |
| `fix`      | 버그 수정                 | 로직 오류, 예외 처리 수정       |
| `docs`     | 문서 수정                 | README, API 명세서              |
| `style`    | 코드 스타일 (의미 변화 X) | 포매팅, 세미콜론                |
| `design`   | UI 디자인 변경            | 프론트엔드와 통일을 위해 유지   |
| `test`     | 테스트 코드               | 테스트 추가/수정                |
| `refactor` | 리팩토링 (기능 변화 X)    | 구조 개선                       |
| `ci`       | CI 설정 수정              | GitHub Actions workflow         |
| `perf`     | 성능 개선                 | 쿼리 최적화, 응답 속도 개선     |
| `chore`    | 자잘한 수정/설정          | 의존성, 빌드 설정, 도구 설정    |
...
예시: `feat/login-12`, `fix/token-expire-34`
